...
export function setupBroadcaster(_options: Schema): Rule {
    ...
    let answers = await inquirer.prompt([
      {
        ...
        name: 'service',
        message:
          "Select the service which's broadcaster you would like to setup",
        choices: services
      }
    ]);
...
function addDeclarationToApp(service: string, _options: Schema): Rule {
  return (tree: Tree, _context: SchematicContext) => {
    ...
    // Search for the last broadcaster setup require expression
    const lastRequireExpression = children
      .filter((x) =>
        matchTextFromNode(x, /\.\/src\/socket\/broadcaster\/broadcaster/)
      )
      .pop();

    // If there is a broadcaster setup declared in the app
    if (lastRequireExpression) {
      throw new SchematicsException('BBroadcaster setup was already complete');
    }
...
function _getSetupDeclarationToInsertContent(_options: Schema): string {
  ...
  // TODO: Be aware of socket and catcher setup
