...
let generate = async (basePath, command, args) => {
  ...
  if (command === 'service') {
    await serviceGenerator.generate(basePath, args.name);
  } else if (command === 'resource') {
    await resourceGenerator.generate(basePath, args.name, args.service);
  }
};
