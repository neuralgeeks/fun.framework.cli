...
let renderResourceTemplate = (templatePath, outPath, resource) => {
  const data = resourceToData(resource);

  renderTemplateFile(templatePath, data).then((renderedString) => {
    createFile(outPath, renderedString);
  });
};

// Renders a .fun-template template into a .js file
let renderTemplate = (templatePath, outPath, data) => {
  renderTemplateFile(templatePath, data).then((renderedString) => {
    createFile(outPath, renderedString);
  });
};
